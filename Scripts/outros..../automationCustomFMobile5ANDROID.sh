...
VersionCode: $versionCode na data $data
  " >>info.txt
    ...
    printf "
		...
		" >>info_"$projeto".html
    mv ./info_"$projeto".html $pathToAAB
  ...
  echo -e "$pathToAAB/Projetos/"$projeto" "
...
builddingApkAndAAB() {
  ...
  mv $pathProject/app/build/outputs/bundle/release/$BUILD_AAB_UNSIGNED.aab $pathToAAB

  cd $pathToAAB
  print_blue "\n\n-> Assinando o .aab ($BUILD_AAB_UNSIGNED.aab) \nApós assinado estára em: $pathToAAB/$BUILD.aab\n\n"
...
movingProject() {
  ...
  if [[ $newProjectOrUpdate == 'novo' || $newProjectOrUpdate == 'n' ]]; then
    ...
    mkdir ./Projetos/"$projeto"
    mkdir ./Projetos/"$projeto"/KEYSTORE
    mkdir ./Projetos/"$projeto"/OUTROS
    mkdir ./Projetos/"$projeto"/BUILDS
    mkdir ./Projetos/"$projeto"/BUILDS/Android
    mkdir ./Projetos/"$projeto"/BUILDS/IOS
    mkdir ./Projetos/"$projeto"/IMAGENS
    mkdir ./Projetos/"$projeto"/IMAGENS/ImagensDoProjeto
    mkdir ./Projetos/"$projeto"/IMAGENS/prints
    mkdir ./Projetos/"$projeto"/IMAGENS/prints/Android
    mkdir ./Projetos/"$projeto"/IMAGENS/prints/IOS
    mkdir ./Projetos/"$projeto"/IMAGENS/prints/IOS/iPhone_5.5
    mkdir ./Projetos/"$projeto"/IMAGENS/prints/IOS/iPhone_6.5
    mkdir ./Projetos/"$projeto"/IMAGENS/prints/IOS/iPad_12.9

    echo -e "\n\n>>> MOVENDO... \nEm: $(pwd)\n"
    mv ./Imagens/* ./Projetos/"$projeto"/"IMAGENS"/"ImagensDoProjeto"
    ...
    mv ./$BUILD.aab ./Projetos/"$projeto"/"BUILDS"/Android/"$nome"_"$versionCode".aab
    mv ./$BUILD.apk ./Projetos/"$projeto"/"BUILDS"/Android/"$nome"_"$versionCode".apk
...
uploadProjectToSeverFileSystem() {
  ...
  # pathToBuildsAndroid=$pathToAAB/Projetos/"$projeto"/BUILDS/Android # Path Correto: /Users/macbook-estagio/Desktop/matheus/trabalho/EXECUTAVEIS/Scripts/AABTolls/Projetos/5977_Invictus\ Control/BUILDS/Android
  ...
  pathToBuildsAndroid=$pathToAAB/Projetos/"$projeto"/BUILDS/Android

  if [[ $newProjectOrUpdate == 'novo' || $newProjectOrUpdate == 'n' ]]; then
    print_blue "\n\n-> Subindo o projeto TODO!"
    echo -e "\n-> SENHA para acessar o servidor: $(print_green "yTYa@V@QJrtP")\n"
    scp -r $pathToAAB/Projetos/"$projeto"/ matheussantos@192.168.1.14:/backups/Arquivos/Mobile/FMobile/FMobile_6/Customs
  else
