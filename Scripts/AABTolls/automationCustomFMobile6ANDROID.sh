...
# read projeto
# pathPorjeto="Scripts/AABTolls/Projetos/$projeto/BUILDS/"
...
enteringTheDatas() {
  ...
  if [[ $newProjectOrUpdate == 'novo' || $newProjectOrUpdate == 'n' || $newProjectOrUpdate == 'N' ]]; then
    ...
    rm $pathToAAB/info.txt
    rm $pathToAAB/info_"$projeto".html
...
nomeDoApp=$(echo "$nome" | awk '{sub(/ /,""); print}')
branch=$(echo "custom $nomeDoApp" | awk '{sub(/ /,""); print}')
echo -e "\n\nChamando gitCheck.sh com a branch: $(print_light_red "$branch")"
sleep 3

sh $pathScripts/Automacao\ Android/gitCheck.sh $newProjectOrUpdate $pathProject $bundle $branch
...
CreateDirectoryToNewProject() {
  if [[ $newProjectOrUpdate == 'novo' || $newProjectOrUpdate == 'n' || $newProjectOrUpdate == 'N' ]]; then
    ...
    mkdir ./Projetos/"$projeto"/IMAGENS/prints/Android/Loja
...
buildName=$(echo -e "$nome" | awk '{sub(/ /,"_"); print}')
